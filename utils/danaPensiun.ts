...
import {
  BreakdownInvestasi,
  InputKalkulatorPensiun,
  Investasi,
  OutputKalkulatorPensiun,
} from "@/constants/Types";

function kalkulatorDanaPensiun(
  input: InputKalkulatorPensiun,
): OutputKalkulatorPensiun {
  const {
    pengeluaranPerBulan,
    usiaSekarang,
  ...
  if (!strategiCocok) {
    ...
    const totalPokokRekomendasiNominal =
      danaPensiunSaatIni + rekomendasiInvestasiPerBulan * n;
    ...
    const totalPokokRekomendasiDurasi =
      danaPensiunSaatIni + targetInvestasiPerBulan * rekomendasiDurasiInvestasi;
    ...
    let tempFVReturn =
      danaPensiunSaatIni * Math.pow(1 + tempReturnInvestasi / 100 / 12, n);
    ...
    const totalPokokRekomendasiReturnBaru =
      danaPensiunSaatIni + targetInvestasiPerBulan * n;
    ...
    rekomendasiPilihanInvestasi = pilihanInvestasi.filter(
      (inv) =>
        inv.returnTahunan >= targetReturnInvestasi - 2 &&
        inv.returnTahunan <= targetReturnInvestasi + 2,
    );
  }
